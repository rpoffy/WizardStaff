...
| Steam leave/rejoin lifecycle cleanup | **Implemented but unverified** | Before a new lobby search, the join path now destroys any stale local `GameSession` and continues only from the matching destroy callback. Cancel/timeout invalidates that continuation. `WizardStaffEditor` built successfully on 2026-08-01; a two-account leave/rejoin retry remains required. |
