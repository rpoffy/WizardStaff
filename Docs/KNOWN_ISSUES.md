...
| Versioned Steam session compatibility | **Implemented but unverified** | A two-account Build `24636896` test found matching readable identities (`0.1.0-private.2-058D96CE`) but different numeric lobby-result `BuildUniqueId` values, so the strict numeric check falsely rejected both players. Search now treats the exact map plus readable version/checksum metadata as the discovery contract and leaves numeric identity diagnostic; Unreal's handshake still rejects incompatible network protocols. Package a replacement and verify matching clients connect while an older private build remains rejected. |
