...
| Version Steam sessions with explicit readable checksum metadata | `WIZARDSTAFF_BUILD` combines explicit `ProjectVersion` with Unreal's network checksum. Steam was observed translating the numeric lobby-result `BuildUniqueId` despite identical packaged clients, so discovery requires exact map and readable build metadata while treating the numeric field as diagnostic. Unreal's connection handshake remains the protocol guard; network-incompatible packages must bump `ProjectVersion`. |
